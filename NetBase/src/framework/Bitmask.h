#ifndef _NETBASE_FRAMEWORK_BITMASK_H_
#define _NETBASE_FRAMEWORK_BITMASK_H_
...
namespace Bitmask
{
	...
	template<typename T>
	inline constexpr T operator~(T rhs) noexcept requires EnableBitmaskOperators<T>::value
	{
		return static_cast<T>(~std::to_underlying(rhs));
	}

	template<typename T>
	inline constexpr T operator|(T lhs, T rhs) noexcept requires EnableBitmaskOperators<T>::value
	{
	...
	template<typename T>
	inline constexpr T operator<<(T lhs, int rhs) noexcept requires EnableBitmaskOperators<T>::value
	{
	...
	template<typename T>
	inline constexpr bool HasFlag(T value, T flag) noexcept requires EnableBitmaskOperators<T>::value
	{
